  ...
  <v-container>
    <v-row justify-sm="center">
      ...
      <v-col cols="12" :sm="9" :md="6">
        ...
        <div>
          <v-btn class="mb-4" block href="https://drive.google.com/open?id=1W5Xo3zw46IFUHJUEgPrLfiImVAS9KvVD">
...
export default {
  name: "rules",
  layout: 'doubletab-rules',
  head() {
    const title = 'Double Tab Rules';
    const description = "Check the rules.";
    const image = '/img/double-tab/doubletab-learn.jpg';
    return {
      title,
      meta: [
  ...
  async asyncData({ $axios }) {
    const { data } = await $axios(`/api/double-tab/rules`);
    return {
      rules: data,
    };
  },
