  ...
  <v-container>
    <v-row justify-sm="center">
      ...
      <v-col cols="12" :sm="9" :md="6">
        ...
        <div>
          <v-btn class="mb-4" block href="https://drive.google.com/open?id=1CdxDSkZVV1ulXY32q_LDq9jHZR7p7QPR">
...
export default {
  name: "rules",
  layout: 'warstuff-rules',
  head() {
    const title = 'WarStuff Rules';
    const description = "Check the rules.";
    const image = '/img/war-stuff/warstuff-learn.jpg';
    return {
      title,
      meta: [
  ...
  async asyncData({ $axios }) {
    const { data } = await $axios(`/api/war-stuff/rules`);
    return {
      rules: data,
    };
  },
