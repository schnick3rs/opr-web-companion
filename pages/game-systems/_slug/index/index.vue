<template>
  <v-row align="stretch">
    <template v-if="layout === 'tiles'">
      ...
      >
        <v-card style="height: 100%">
          <span v-show="false" class="wip-band">Autogenerated</span>
    ...
    <v-dialog
      ...
      width="fit-content"
    >
      <opr-army-book :army-book="previewArmyBook" />
    </v-dialog>
  </v-row>
</template>
...
export default {
  ...
  async asyncData({ $axios, params }) {
    const { slug } = params;
    const { data: gameSystem } = await $axios.get(`/api/game-systems/${slug}`);
    const { data: armyBooks } = await $axios.get('/api/army-books/', {
      params: {
        gameSystemSlug: gameSystem.slug,
        filters: ['official'],
      },
    });
    return {
    ...
    groupedArmyBooks(armyBooks = []) {
      armyBooks = armyBooks.reduce((previousValue, currentValue, currentIndex, array) => {
        if (currentValue.factionName) {
          ...
          const index = previousValue.findIndex(item => item._type === 'faction' && item.name === currentValue.factionName);
